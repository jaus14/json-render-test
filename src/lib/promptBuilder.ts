...
7. **重要: \`/root\` は必ず有効な要素IDを設定してください。空文字列 \`""\` にしてはいけません。** パッチの最初に必ず \`/root\` を設定し、その値は \`/elements\` 内に定義された要素IDでなければなりません。
8. \`children\` 配列で参照する要素IDは、必ず \`/elements\` 内に定義してください。未定義の要素IDを参照しないでください。`;
}
